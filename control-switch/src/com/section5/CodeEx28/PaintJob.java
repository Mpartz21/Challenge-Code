...
public class PaintJob {
    public static int getBucketCount(double width,double height,double areaPerBucket,int extraBuckets){
        ...
        double amountAvaliable = areaPerBucket * extraBuckets;
        double amountNeeded = area - amountAvaliable;

        if(width <= 0||height <= 0||areaPerBucket <= 0||extraBuckets < 0){
            bucketsToBuy = -1;
            return bucketsToBuy;
        }
        bucketsToBuy = (int) Math.ceil(amountNeeded / areaPerBucket);
    ...
    public static void main(String[] args) {
        int test = getBucketCount(3.4,2.1,1.5,2);

...
/*
Write a method named area with one double parameter renamed radius.
...
public class AreaCalculator {
    ...
    public static double area (double radius){
        double areaOfTheCircle = 0.0D;

        if (radius < 0){
            areaOfTheCircle = -1.0D;
            return  areaOfTheCircle;
        }else {
            areaOfTheCircle = Math.PI * (radius * radius);
        }
    ...
    public static double area ( double x, double y){
        double areaOfTheRectangle = 0.0D;

        if(x < 0 || y < 0 ){
            areaOfTheRectangle = -1.0D;
